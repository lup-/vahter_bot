...
class Injector {
    constructor(bot) {
        this.bot = bot;
        this.app = new Telegraf(bot.token);
        this.stage = false;
    }
    ...
    addSubscription() {
        this.app.use((ctx, next) => checkSubscriptionMiddleware(ctx, next, this.bot));
        return this;
    }
    ...
    addDisclaimer(text, afterAccept) {
        this.app.start(async ctx => {
            ...
            try {
                ...
                return ctx.reply(text, menu([{code: '_accept', text: 'Хорошо'}]));
            }
            catch (e) {
                console.log('intro error '+e);
            }
        });
